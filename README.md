...
Для правки текста без изменения React-кода:

- Hero, продукты, метод, условия, финальный CTA: `data/landing-content.json`.
- Кейсы, YouTube-ссылки, задачи и что сделано: `data/works.json`.
- Контакты Telegram/Instagram/VK/email: `src/lib/profile.ts`.

В `/editor` изменения применяются автоматически в предпросмотре и сохраняются
в браузере.

- `Выбрать папку` подключает корень проекта или папку `data`.
- `Сохранить в проект` записывает `data/landing-content.json` и `data/works.json`.
- `Скачать текст` скачивает `data/landing-content.json`.
- `Скачать работы` скачивает `data/works.json`.
- Во вкладке `Видео` можно менять YouTube-ссылки, названия, задачи, описание,
  формат, что сделано и главный шоурил.

В production `/editor` закрыт. Публичные посетители видят только сайт.
Редактор работает только локально через `npm run dev`.
