...
- `Text JSON` скачивает `data/landing-content.json`.
- `Works JSON` скачивает `data/works.json`.
- Во вкладке `Видео` можно менять YouTube-ссылки, названия, задачи, описание,
  формат, что сделано и главный шоурил.
- В Chrome/Edge кнопки `Save text` и `Save works` могут сохранить JSON прямо
  в файл через системный диалог. Выбери соответствующий файл в папке `data`.
